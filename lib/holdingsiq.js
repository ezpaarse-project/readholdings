...
const postHoldings = async (conf) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/holdings - Fail 3 times`);
    process.exit(1);
  }
...
const getHoldingsStatus = async (conf) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/holdings/status - Fail 3 times`);
    process.exit(1);
  }
...
const getHoldings = async (conf, count, offset, index) => {
  ...
  logger.info(`length of result: ${res?.data?.holdings.length}`);
...
const parseGetVendorsPackagesTitles = (data) => ({
  ...
  CoveragePackage: data?.customerResourcesList?.[0]?.isPackageCustom,
  ProxyID: getProxy(data?.customerResourcesList?.[0]?.proxy?.id),
  TitleSelected: data?.customerResourcesList?.[0]?.isSelected,
});
...
const getVendorsPackagesTitles = async (conf, vendorID, packageID, kbID) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/vendors/${vendorID}/packages/${packageID}/titles/${kbID} - Fail 3 times`);
    process.exit(1);
  }
...
const getVendorsPackages = async (conf, vendorID, packageID, index) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/vendors/${vendorID}/packages/${packageID} - Fail 3 times`);
    process.exit(1);
  }
