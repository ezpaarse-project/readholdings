...
const getHoldingsStatus = async (conf) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/holdings/status - Fail 6 times`);
    process.exit(1);
  }
...
const getHoldings = async (conf, count, offset, index) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/holdings - Fail 6 times`);
    process.exit(1);
  }
...
const getVendorsPackagesTitles = async (conf, vendorID, packageID, kbID) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/vendors/${vendorID}/packages/${packageID}/titles/${kbID} - Fail 6 times`);
    process.exit(1);
  }
...
const getVendorsPackages = async (conf, vendorID, packageID, index) => {
  ...
  if (i >= 6) {
    logger.error(`Cannot request /${custid}/vendors/${vendorID}/packages/${packageID} - Fail 6 times`);
    process.exit(1);
  }
